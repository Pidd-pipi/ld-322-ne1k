...
func main() {
	...
	scheduleService := service.NewScheduleService(schedules, devices, l, hub)
	alertService := service.NewAlertService(alerts, l)
	reports := service.NewReportService(sensors, alerts)
	runner := service.NewSchedulerRunner(scheduleService, l)
	go runner.Start(context.Background())
	engine := router.New(router.Dependencies{Config: cfg, Logger: l, Auth: auth, Monitoring: monitoring, Alerts: alertService, Control: control, Schedules: scheduleService, Reports: reports, Hub: hub})
